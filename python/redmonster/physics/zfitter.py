...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def z_refine2(self, threshold=23.3, width=15, num_z=5): # Default threshold of 23.3 is delta (chi_r)**2 = .005, and width is 1000 km/s
        ...
        for ifiber in xrange(self.zchi2.shape[0]):
            ...
            bestminvectors = []
            bestchi2vals = []
            '''bestzvec = self.zchi2[ifiber][ n.unravel_index( self.zchi2[ifiber].argmin(), self.zchi2[ifiber].shape )[:-1] ]
            for iz in xrange(self.zchi2.shape[-1]):
                allminvectors.append( n.unravel_index( self.zchi2[ifiber].argmin(), self.zchi2[ifiber].shape )[:-1] )'''
            bestzvec = n.zeros( self.zchi2.shape[-1] )
